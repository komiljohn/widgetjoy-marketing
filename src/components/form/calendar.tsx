...
const cellStyles = tv({
  base: "w-9 h-9 text-sm cursor-default rounded-full flex items-center justify-center forced-color-adjust-none focus-ring",
  variants: {
    isSelected: {
      false:
...
export function CalendarHeader() {
  ...
  return (
    <header className="flex items-center gap-1 pb-4 px-1 w-full">
      ...
      <Button slot="next">{direction === "rtl" ? <ChevronLeft aria-hidden /> : <ChevronRight aria-hidden />}</Button>
    </header>
  );
}
